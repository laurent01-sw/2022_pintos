...
{
  char *fn_copy, *exe_name, *save_ptr;
  tid_t tid;
...
process_wait (tid_t child_tid) 
{
  ...
  if (t && !t->seen_status) 
  {
    ...
    // printf("PTID : %d, CTID : %d Thread Not Found!\n");
    return -1;
  }
}
...
static bool setup_stack (const char* arg_pointer, void **esp);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  struct file *s_file = NULL, *file = NULL;
  off_t file_ofs;
  bool success = false;
  int i;
  ...
  {
    s_file = filesys_open(exe_name);
    file_deny_write(s_file);
    // printf("(%s) name : %s file : %p inode : %p\n",__func__,exe_name,s_file,s_file->inode);
    t->itself = s_file;
  }
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (const char* arg_pointer, void **esp) 
{
  /*Parse the argument                                                         
  Original string literal is maintained but only delimiter is replaced by \0   
  Each token's address is saved at the arg_save array*/                        
  char *s = arg_pointer;                                                       
  char *token, *save_ptr;                                                      
  uint32_t arg_num = 0;                                                        
  uint32_t i;                                                                       
  ...
  const char* p;                                                               
  for(p = arg_pointer; true ; p++)                                             
  {                                                                            
    if(*p == ' ' || *p == '\0') arg_num++;                                     
    if(*p == '\0') break;                                                      
  }                                                                            
                                                                             
  char *arg_save[arg_num];                                                     
  i = 0;                                                                       
  ...
  uint32_t arg_size[arg_num];                                                  
  uint32_t arg_size_t = 0;                                                     
  for(i = 0; i < arg_num; i++)                                                 
  {                                                                             
    arg_size[i] = strlen(arg_save[i]) + 1;                                     
    arg_size_t += arg_size[i];                                                 
  }                                                                            
                                                                             
  if(arg_size_t % 4) // word-alignment                                         
    arg_size_t += 4 - (arg_size_t % 4);                                        
                                                                             
  void **temp_p;                                                               
  temp_p = (void**)(PHYS_BASE - (8 + 4 * arg_num + arg_size_t));               
  //printf("Temp Address : %x\n",temp_p);                                      
  uint32_t* arg_size_user_t = (uint32_t*)(temp_p - 1);                         
  ...
  if (kpage != NULL) 
    {
      ...
      {
        uint32_t accum = 0;                                                                      
        *arg_size_user_t = arg_num;                                                              
                                                                                         
        for(i = 0; i < arg_num; i++)                                                             
        {                                                                                        
          accum += arg_size[arg_num - i - 1];                                                    
